    ...
    let addTask = await inquirer.prompt([
        {
            name: "todo",
            type: "input",
            message: chalk.bold.blueBright("What you want to add in your Todos list?"),
        },
        {
            name: "addMore",
    ...
    if (addTask.addMore) {
        let tododeleted = await inquirer.prompt([
            {
                name: "delete",
                type: "confirm",
                message: chalk.bold.yellowBright("Do you want to delete anything in todo?" // yes or no
                ),
                default: "false",
            },
        ]);
        if (tododeleted.delete) {
            let deleteAnswer = await inquirer.prompt([
                {
                    name: "list",
                    message: chalk.bold.greenBright("Choose an element to delete:"),
                    type: "options",
                    choices: todos,
                },
            ]);
            let index = todos.indexOf(deleteAnswer.index);
